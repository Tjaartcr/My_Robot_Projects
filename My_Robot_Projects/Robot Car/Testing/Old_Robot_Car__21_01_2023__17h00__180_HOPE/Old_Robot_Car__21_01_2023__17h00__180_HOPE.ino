...
int Distreading5TurnL = 0;  // BOTTOM MIDDEL SCANNER
int Distreading5TurnR = 0;  // BOTTOM MIDDEL SCANNER
...
#define rightPin2A 8
#define rightPinEN 50
#define rightPin2B 51
...
//ScanRear();  
  ...
  while ((Distreading4 >= FarSetPoint4) && (Entry ==  LOW))  {
...
void MoveForwardEnterAI() {
  ...
  //if (Distreading5TurnL <= Distreading5TurnR){
  //MoveStopAI();
  ////MoveLeft90AI();
  ////MoveStopAI();
...
void ScanForward() {
  ...
  if ((AngleM <= 100) && (AngleM >= 80)) {
    ...
    Distreading4 = (FrontSensor.distanceInCm() * 10);
  }
...
void BackScanLeft() {
  ...
  Distreading5L  = (BackSensor.distanceInCm() * 10);
...
void FrontScanLeft180() {
  ...
  Distreading5TurnL  = (FrontSensor.distanceInCm() * 10);
  digitalWrite(buzzerPin, HIGH);

  delay(100);

  //  DifferanceLeftTurnU180  = ((Distreading5TurnL >= Distreading5TurnL + 100) || (Distreading5TurnL <= Distreading5TurnL - 100));


  lcd.setCursor(8, 0);
  lcd.print(Distreading5TurnL);
  lcd.print(" ");
...
void BackScanRight() {
  ...
  Distreading5R  = (BackSensor.distanceInCm() * 10);
  //  Distreading5TurnR  = (BackSensor.distanceInCm() * 10);
...
void FrontScanRight180() {
  ...
  myservoScanFront.write(0);

  delay(1500);

  Distreading5TurnR = (FrontSensor.distanceInCm() * 10);

  //DifferanceRightTurnU180  = ((Distreading5TurnR >= Distreading5TurnR + 100) && (Distreading5TurnR <= Distreading5TurnR - 100));
...
void setup() {
  ...
  myservoScanBack.attach(10);  // create servo object to control a servo
  myservoScanFront.attach(9);  // create servo object to control a servo
...
void loop() {
  ...
  Distreading4 = (FrontSensor.distanceInCm() * 10);  // FRONT SCANNER
  Distreading4L = 0;  // BOTTOM MIDDEL SCANNER
  Distreading4R = 0;  // BOTTOM MIDDEL SCANNER

  Distreading5 = (BackSensor.distanceInCm() * 10);  // REAR SENSOR
  Distreading5L = 0;  // BOTTOM MIDDEL SCANNER
  Distreading5R = 0;  // BOTTOM MIDDEL SCANNER

  Distreading5TurnL = 0;  // BOTTOM MIDDEL SCANNER
  Distreading5TurnR = 0;  // BOTTOM MIDDEL SCANNER
...
//              if ( Entry ==  LOW){
    if ((Distreading4 >= FarSetPoint4) && ( Entry ==  LOW)) {
      MoveForwardAI();
        LeftUTurn180CNTOn = LOW;
        RightUTurn180CNTOn = LOW;      
    }
//              }
    else if ((Distreading4 <= CloseSetPoint4) || ( Entry ==  HIGH)) {
      MoveStopAI();
    }
    ...
    if (Distreading5TurnL <= Distreading5TurnR){
     TurnMoveLeft90AI();
    }else if (Distreading5TurnL >= Distreading5TurnR){
     TurnMoveRight90AI();
    }
                MoveStopAI();
              MoveForwardEnterTurnAI();
...
//      FrontScanRight180();
      ...
      lcd1.setCursor(15, 0);
      lcd1.print(Distreading5TurnL);
      lcd1.print(" ");

      lcd1.setCursor(7, 0);
      lcd1.print(Distreading5TurnR);
      lcd1.print(" ");

//            if ((Distreading5TurnL <= Distreading5TurnR) && (LeftUTurn180CNT <= 0)) {
//      if ((Distreading4 <= FarSetPoint4) && ((LeftUTurn180CNT == 1) && (RightUTurn180CNT == 0))) {
//      if ((Distreading4 <= FarSetPoint4) && ((LeftUTurn180CNT == 1))) {
      if (Distreading4 <= FarSetPoint4){
...
//      if ((Distreading5TurnL >= Distreading5TurnR) && (RightUTurn180CNT <= 0)) {
//        if ((Distreading4 >= FarSetPoint4) && ((LeftUTurn180CNT == 0) && (RightUTurn180CNT == 1))) {
//        if ((Distreading4 >= FarSetPoint4) && ((LeftUTurn180CNT == 0))) {
        if (Distreading4 >= FarSetPoint4){
...
//        RightUTurn180CNTOn = LOW;
      ...
      //          delay(500);
      //
      //          Distreading4L = (FrontSensor.distanceInCm() * 10);  // BOTTOM MIDDEL SCANNER
      //
      //          lcd.setCursor(11, 1);
      ...
      //          delay(1000);
      //
      //          Distreading4R = (FrontSensor.distanceInCm() * 10);  // BOTTOM MIDDEL SCANNER
      //
      //          lcd.setCursor(3, 1);
